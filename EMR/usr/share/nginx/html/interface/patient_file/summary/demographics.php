...
<?php
  ...
  $result4 = sqlStatement("SELECT fe.encounter,fe.date,isemr_postcalendar_categories.pc_catname FROM form_encounter AS fe ".
    " left join isemr_postcalendar_categories on fe.pc_catid=isemr_postcalendar_categories.pc_catid  WHERE fe.pid = ? order by fe.date desc", array($pid));
...
<?php echo xlt('External Data'); ?></a>
      |
      <a href="/api/transfer.php?pid=<?php echo attr($pid);?>">Transfer</a>
 ...
 <?php  }  // close advanced dir block
	...
	if (isset($pid) && !$GLOBALS['disable_calendar'] && $showpast > 0) {
	 $query = "SELECT e.pc_eid, e.pc_aid, e.pc_title, e.pc_eventDate, " .
	  ...
	  "FROM isemr_postcalendar_events AS e, users AS u, " .
	  "isemr_postcalendar_categories AS c WHERE " .
