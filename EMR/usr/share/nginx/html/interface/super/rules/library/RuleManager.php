...
class RuleManager {
    ...
    const SQL_isemr_rule_reminder_INTERVAL =
    "SELECT id,
            method,
            method_detail,
            value
       FROM isemr_rule_reminder
      WHERE id = ?";
    ...
    "DELETE FROM isemr_rule_reminder
           WHERE id = ?";

    const SQL_INSERT_INTERVALS =
    "INSERT INTO isemr_rule_reminder
    ...
    /**
     * Creates a ReminderIntervals object from rows in the isemr_rule_reminder table,
     * and sets it in the supplied Rule.
     * @param Rule $rule
     */
    private function fillRuleReminderIntervals( $rule ) {
        $stmt = sqlStatement( self::SQL_isemr_rule_reminder_INTERVAL, array( $rule->id ) );
