...
function InsertEventFull()
 {
	...
	// =======================================
        if (is_array($_POST['form_provider'])) {
            ...
            $q = sqlStatement ("SELECT MAX(pc_multiple) as max FROM isemr_postcalendar_events");
            $max = sqlFetchArray($q);
...
if ( $eid ) {
    ...
    $facility = sqlQuery("SELECT pc_facility, pc_multiple, pc_aid, facility.name
                            FROM isemr_postcalendar_events
                              LEFT JOIN facility ON (isemr_postcalendar_events.pc_facility = facility.id)
    ...
    if ( is_array($facility) && !$facility['pc_facility'] ) {
        ...
        if ( $GLOBALS['select_multi_providers'] ) {
            ...
            sqlStatement("UPDATE isemr_postcalendar_events SET pc_facility = ? WHERE pc_multiple = ?", array($min,$mul) );
        }
        // EOS multiple

        sqlStatement("UPDATE isemr_postcalendar_events SET pc_facility = ? WHERE pc_eid = ?", array($min,$eid) );
        $e2f = $min;
        $e2f_name = $min_name;
    } else {
      // not edit event
...
if ($_POST['form_action'] == "save") {
    ...
    if ($eid) {
        ...
        $row = sqlQuery("SELECT pc_multiple FROM isemr_postcalendar_events WHERE pc_eid = ?", array($eid) );
        ...
        if ($GLOBALS['select_multi_providers'] && $row['pc_multiple']) {
            ...
            $up = sqlStatement("SELECT pc_aid FROM isemr_postcalendar_events WHERE pc_multiple=?", array($row['pc_multiple']) );
            ...
            if ($_POST['recurr_affect'] == 'current') {
                ...
                foreach ($providers_current as $provider) {
                    ...
                    $origEvent = sqlQuery("SELECT pc_recurrspec FROM isemr_postcalendar_events ".
                    ...
                    sqlStatement("UPDATE isemr_postcalendar_events SET " .
                        " pc_recurrspec = ? ".
            ...
            // ===== Future Recurring events of a repeating series =====
                ...
                if (count ($r1)) {
                    foreach ($r1 as $to_be_removed) {
                        sqlQuery("DELETE FROM isemr_postcalendar_events WHERE pc_aid=? AND pc_multiple=?", array($to_be_removed,$row['pc_multiple']) );
                    }
                }
        ...
        // ====================================
            ...
            if ($_POST['recurr_affect'] == 'current') {
                // get the original event's repeat specs
                $origEvent = sqlQuery("SELECT pc_recurrspec FROM isemr_postcalendar_events WHERE pc_eid = ?", array($eid) );
                $oldRecurrspec = unserialize($origEvent['pc_recurrspec']);
                $selected_date = date("Ymd", strtotime($_POST['selected_date']));
                if ($oldRecurrspec['exdate'] != "") { $oldRecurrspec['exdate'] .= ",".$selected_date; }
                else { $oldRecurrspec['exdate'] .= $selected_date; }
...
// =======================================
        ...
        if ($GLOBALS['select_multi_providers']) {
            ...
            if ($_POST['recurr_affect'] == 'current') {
                ...
                sqlStatement("DELETE FROM isemr_postcalendar_events WHERE ".$whereClause);
            }
        }
        ...
        // =======================================
            ...
            if ($_POST['recurr_affect'] == 'current') {
                ...
                sqlStatement("DELETE FROM isemr_postcalendar_events WHERE pc_eid = ?", array($eid) );
            }
        }
 }
...
*********************************************************************/
 ...
 if ($eid) {
  // $row = sqlQuery("SELECT * FROM isemr_postcalendar_events WHERE pc_eid = $eid");
 ...
 $cres = sqlStatement("SELECT pc_catid, pc_catname, pc_recurrtype, pc_duration, pc_end_all_day " .
  "FROM isemr_postcalendar_categories ORDER BY pc_catname");
...
if  ($GLOBALS['select_multi_providers']) {
    ...
    if ($eid) {
        if ( $multiple_value ) {
            ...
            $qall = sqlStatement ("SELECT pc_aid AS providers FROM isemr_postcalendar_events WHERE pc_multiple = ?", array($multiple_value) );
            while ($r = sqlFetchArray($qall)) {
                $providers_array[] = $r['providers'];
            }
        } else {
            $qall = sqlStatement ("SELECT pc_aid AS providers FROM isemr_postcalendar_events WHERE pc_eid = ?", array($eid) );
...
// =======================================
    ...
    if ($eid) {
        ...
        $qprov = sqlStatement ("SELECT pc_aid FROM isemr_postcalendar_events WHERE pc_eid = ?", array($eid) );
