...
function manage_tracker_status($apptdate,$appttime,$eid,$pid,$user,$status='',$room='',$enc_id='') {
  ...
  $pc_appt =  sqlQuery("SELECT `pc_recurrtype` FROM `isemr_postcalendar_events` WHERE `pc_eid` = ?", array($eid));
  ...
  $pc_appt =  sqlQuery("SELECT `pc_apptstatus`, `pc_room` FROM `isemr_postcalendar_events` WHERE `pc_eid` = ?", array($eid));
  if ($status != $pc_appt['pc_apptstatus']) {
    sqlStatement("UPDATE `isemr_postcalendar_events` SET `pc_apptstatus` = ? WHERE `pc_eid` = ?", array($status,$eid));
  }
  if ($room != $pc_appt['pc_room']) {
    sqlStatement("UPDATE `isemr_postcalendar_events` SET `pc_room` = ? WHERE `pc_eid` = ?", array($room,$eid));
  }
