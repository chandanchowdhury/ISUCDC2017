...
function clinical_summary_widget($patient_id,$mode,$dateTarget='',$organize_mode='default',$user='') {
  ...
  foreach ($actions as $action) {
    ...
    if ($action['due_status']) {
      ...
      echo generate_display_field(array('data_type'=>'1','list_id'=>'isemr_rule_reminder_due_opt'),$action['due_status']) . "</span>)";
    }
  ...
  foreach ($actions as $action) {
    ...
    if ($action['due_status']) {
      ...
      if ($action['due_status'] == "past_due") {
        ...
        $returnOutput .= generate_display_field(array('data_type'=>'1','list_id'=>'isemr_rule_reminder_due_opt'),$action['due_status']) . "</span>)<br>";
    }
    else {
      $returnOutput .= "<br>";
    }
...
function test_rules_clinic($provider='',$type='',$dateTarget='',$mode='',$patient_id='',$plan='',$organize_mode='default',$options=array(),$pat_prov_rel='primary',$start=NULL,$batchSize=NULL,$user='') {
  ...
  foreach( $rules as $rowRule ) {
    ...
    //  from isemr_rule_reminder table
    $target_dates = array();
    if ($mode != "report") {
...
function resolve_reminder_sql($rule,$reminder_method) {
  $sql = sqlStatementCdrEngine("SELECT `method_detail`, `value` FROM `isemr_rule_reminder` " .
...
function appointment_check($patient_id,$dateTarget='') {
  ...
  $sql = sqlStatementCdrEngine("SELECT isemr_postcalendar_events.pc_eid, " .
    "isemr_postcalendar_events.pc_title, " .
    "isemr_postcalendar_events.pc_eventDate, " .
    "isemr_postcalendar_events.pc_startTime, " .
    "isemr_postcalendar_events.pc_endTime " .
    "FROM isemr_postcalendar_events " .
    "WHERE isemr_postcalendar_events.pc_eventDate > ? " .
    "AND isemr_postcalendar_events.pc_eventDate <= ? " .
    "AND isemr_postcalendar_events.pc_pid = ?", array($currentDate,$dateTarget,$patient_id) );
