...
function calendar_arrived($form_pid) {
	...
	$result_event=sqlStatement("SELECT * FROM isemr_postcalendar_events WHERE pc_recurrtype != '0' and pc_pid = ? and pc_endDate != '0000-00-00'
		and pc_eventDate < ? and pc_endDate >= ? ",
		array($form_pid,$Today,$Today));
	if(sqlNumRows($result_event)==0)//no repeating appointment
	 {
		$result_event=sqlStatement("SELECT * FROM isemr_postcalendar_events WHERE pc_pid =?	and pc_eventDate = ?",
		...
		if(sqlNumRows($result_event)==0)//no appointment
		...
		else//one appointment
		 {
			 ...
			 $zero_enc=0;
			 sqlStatement("UPDATE isemr_postcalendar_events SET pc_apptstatus ='@' WHERE pc_pid =? and pc_eventDate = ?",
				 array($form_pid,$Today));
		 }
	 }
			...
			while(1)
			 {
				if($pc_eventDate==$Today)//Matches so insert.
				 ...
				 if(!$exist_eid=check_event_exist($pc_eid))
					...
					{
					 sqlStatement("UPDATE isemr_postcalendar_events SET pc_apptstatus = '@' WHERE pc_eid = ?",
						 array($exist_eid));
					}
					 $enc = todaysEncounterCheck($form_pid);//create encounter
					 $zero_enc=0;
...
function update_event($eid)
 {
	$origEventRes = sqlStatement("SELECT * FROM isemr_postcalendar_events WHERE pc_eid = ?",array($eid));
	...
	sqlStatement("UPDATE isemr_postcalendar_events SET pc_recurrspec = ? WHERE pc_eid = ?",array(serialize($oldRecurrspec),$eid));
...
function check_event_exist($eid)
 {
	...
	$origEvent = sqlStatement("SELECT * FROM isemr_postcalendar_events WHERE pc_eid != ? and pc_catid=? and pc_aid=? ".
...
function InsertEvent($args,$from = 'general') {
  ...
  $form_room = empty($args['form_room']) ? '' : $args['form_room'];
	...
	if($from == 'general'){
    $pc_eid = sqlInsert("INSERT INTO isemr_postcalendar_events ( " .
		...
		sqlStatement("INSERT INTO isemr_postcalendar_events ( " .
