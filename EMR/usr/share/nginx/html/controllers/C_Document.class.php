...
class C_Document extends Controller {
	...
	function view_action($patient_id="",$doc_id) {
		...
		$result_docs = sqlStatement("SELECT fe.encounter,fe.date,isemr_postcalendar_categories.pc_catname FROM form_encounter AS fe " .
			"LEFT JOIN isemr_postcalendar_categories ON fe.pc_catid=isemr_postcalendar_categories.pc_catid  WHERE fe.pid = ? ORDER BY fe.date desc",array($patient_id));
		...
		$cres = sqlStatement("SELECT pc_catid, pc_catname FROM isemr_postcalendar_categories ORDER BY pc_catname");
