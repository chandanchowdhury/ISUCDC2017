...
//actual execution is done in the select_query function in Server_side
    ...
    public function query_formation($data){
	    ...
	    //master data for calendar from Globals
            ...
            array_unshift($data[1],$pid);
            $query="select pc_eid,pc_eventDate,pc_startTime,pc_endTime,fname,lname,name,pc_apptstatus from isemr_postcalendar_events AS c,
            ...
            $query="select pc_duration from isemr_postcalendar_categories WHERE pc_catid = ?";
            return array($query,$data[1]);
            break;
            ...
            pc_prefcatid from isemr_postcalendar_events WHERE pc_aid = ? AND ((pc_endDate >= ? AND pc_eventDate < ?) OR
