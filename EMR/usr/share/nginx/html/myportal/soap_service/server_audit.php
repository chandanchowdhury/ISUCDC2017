		 ...
		 if($rowfield['pid']>0)
		  {
			...
			$qry = "DELETE from patient_access_offsite WHERE  pid=? ";
		        sqlStatement($qry,array($pid));
			$qry = "DELETE from isemr_postcalendar_events WHERE  pc_pid=? ";// appointments approved, but patient denied case.
		        sqlStatement($qry,array($pid));
  ...
  public function update_audited_data($var)
       {
	      $data_credentials=$var[0];
				...
				$validtables = array("patient_data","employer_data","insurance_data","history_data","isemr_postcalendar_events","ar_session","documents_legal_master","documents_legal_detail","patient_access_offsite");
        if(UserService::valid($data_credentials)){
	      $audit_master_id = $var['audit_master_id'];
	      ...
	      while($row = sqlFetchArray($res)){
		     ...
		     if($cnt>0){
			    while($rowfield = sqlFetchArray($resfield)){
				  ...
				  if($table=='isemr_postcalendar_events'){
				    $newdata['isemr_postcalendar_events'][$rowfield['field_name']]=$rowfield['field_value'];
				  }
			    ...
			    elseif($table=='isemr_postcalendar_events'){
			      sqlInsert("INSERT INTO isemr_postcalendar_events ( " .
				    ...
				    "'" . add_escape_custom($newdata['isemr_postcalendar_events']['pc_title'])           . "', " .
				    "NOW(), "                                         .
				    "'" . add_escape_custom($newdata['isemr_postcalendar_events']['pc_hometext']) . "', " .
				    "'" . add_escape_custom($newdata['isemr_postcalendar_events']['pc_eventDate'])          . "', " .
				    "'" . add_escape_custom($newdata['isemr_postcalendar_events']['pc_endDate'])                        . "', " .
				    "'" . add_escape_custom($newdata['isemr_postcalendar_events']['pc_startTime'])     . "', " .
				    "'" . add_escape_custom($newdata['isemr_postcalendar_events']['pc_endTime'])                   . "', " .
				    "'" . add_escape_custom($newdata['isemr_postcalendar_events']['pc_duration']) . "', " .
				    "'" . add_escape_custom($newdata['isemr_postcalendar_events']['pc_catid'])             . "', " .
				    "1, " .
				    "'" . add_escape_custom($newdata['isemr_postcalendar_events']['pc_aid'])."', " .
				    "'" . add_escape_custom($newdata['isemr_postcalendar_events']['pc_facility'])               . "')"
				);
			    }
