...
/*if ( $eid ) {
    $selfacil = '';
    $facility = sqlQuery("SELECT pc_facility, pc_multiple FROM isemr_postcalendar_events WHERE pc_eid = $eid");
    if ( !$facility['pc_facility'] ) {
        ...
        if ( $GLOBALS['select_multi_providers'] ) {
            ...
            sqlStatement("UPDATE isemr_postcalendar_events SET pc_facility = $min WHERE pc_multiple = $mul");
        }
        // EOS multiple

        sqlStatement("UPDATE isemr_postcalendar_events SET pc_facility = $min WHERE pc_eid = $eid");
        $e2f = $minId;
    } else {
...
========================================================*/
  if ($eid) {
    ...
    $row = sqlQuery("SELECT pc_multiple FROM isemr_postcalendar_events WHERE pc_eid = $eid");
    ...
    if ($GLOBALS['select_multi_providers'] && $row['pc_multiple']) {
        ...
        $up = sqlStatement("SELECT pc_aid FROM isemr_postcalendar_events WHERE pc_multiple={$row['pc_multiple']}");
        ...
        if (count ($r1)) {
            ...
            sqlQuery("DELETE FROM isemr_postcalendar_events WHERE pc_aid='$to_be_removed' AND pc_multiple={$row['pc_multiple']}");
            }
        }
    ...
    if ($tmprow['count'] == 0) {
      ...
      $info_msg .= "New encounter $encounter was created. ";
        ...
        if ($GLOBALS['select_multi_providers']) {
            ...
            $row = sqlQuery("SELECT pc_multiple FROM isemr_postcalendar_events WHERE pc_eid = $eid");
			if ( $row['pc_multiple'] ) {
				sqlStatement("DELETE FROM isemr_postcalendar_events WHERE pc_multiple = {$row['pc_multiple']}");
			} else {
                                sqlStatement("DELETE FROM isemr_postcalendar_events WHERE pc_eid = $eid");
                        }
        // =======================================
        //  EOS multi providers case
        // =======================================
